...
// Mapped from db.ts dbConfig fallbacks
const dbConfig = {
...
// SQL helper to match mysqlToPostgresQuery
function pgSql(sql) {
    let index = 1;
    let tempSql = '';
...
async function run() {
    ...
    try {
        // Asegurar que las columnas de auditoría y secuencias de Fase 3 y Fase 4 existan en base de datos
        await conn.query(pgSql("CREATE SEQUENCE IF NOT EXISTS public.order_number_seq START WITH 1000;"));
        await conn.query(pgSql("ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS order_number VARCHAR(50) UNIQUE;"));
        await conn.query(pgSql("ALTER TABLE public.inventory_movements ADD COLUMN IF NOT EXISTS performed_by VARCHAR(100) NULL;"));
        await conn.query(pgSql("ALTER TABLE public.inventory_movements ADD COLUMN IF NOT EXISTS reason TEXT NULL;"));
        ...
        if (catRes.rows.length === 0) {
            // Si no hay categorías, insertamos una temporal para el test
            const tempCatId = 999;
