...
- [x] Rate limiting activo y probado en login (5/15min IP+email, Fase 9 — probado con 6 intentos: el 6to se rechaza aunque la contraseña sea correcta), registro (10/hora IP), recuperar contraseña (3/hora email), checkout (20/min usuario — la aplicación de cupones vive dentro de esta misma Server Action, no es un endpoint separado) y geocodificación de direcciones (10/min usuario). Implementado con Upstash Redis (`lib/rate-limit/limiter.ts`), fail-open si Upstash no está configurado (no debe llegar así a producción). Pendiente para una pasada futura: límites genéricos de `/admin/**` y `/repartidor/**` (ya protegidos por RBAC/RLS, rate limit ahí es una capa adicional, no la única defensa).
...
- [x] Revisión de que ninguna clave sensible quedó commiteada en el repositorio (`git log -p | grep` de patrones de API key) — hecha en la Fase 9. **Hallazgo pendiente:** el historial de git (heredado de un proyecto no relacionado que ocupaba este mismo repo antes del scaffold de Pan de Rey v2) contiene el Access Token y Public Key de Mercado Pago actuales, hardcodeados en código fuente de ese proyecto viejo — y ese historial ya está pusheado a `origin/master` en GitHub. Ninguna otra credencial actual (Supabase, Resend, OpenRouteService) aparece en el historial. Decisión del cliente: por ahora no rotar ni reescribir el historial — queda pendiente para antes del lanzamiento a producción.
...
11. **Venta por mayor (B2B) — REVISADO, ver nota:** se había definido originalmente como **fuera de este proyecto** (solución separada, sin compartir base de datos). El cliente revirtió esa decisión después de la Fase 7: **sí** se integra con este sistema, como una fase futura todavía sin diseñar en detalle. Definido hasta ahora (a confirmar/expandir cuando se aborde):
    - **Alta:** autoservicio — el cliente aplica (ej. RUT de empresa, giro) y Admin aprueba/rechaza antes de que la cuenta vea precios mayoristas.
    - **Precios:** por tramos de cantidad (similar en espíritu a `shipping_zones` — a más cantidad, mejor precio por unidad), no un descuento plano ni una segunda lista de precios fija por producto.
    - **Pago:** con plazo de crédito (factura a 30 días), no el checkout inmediato de Mercado Pago/transferencia que usa el resto del sitio — esto implica un estado de pedido y un flujo de cobro distintos a los ya construidos (sección 07).
    - **Pendiente de diseñar:** modelo de datos (posible nuevo rol o flag en `profiles`, tabla de tramos de precio mayorista, tabla de aprobaciones), en qué catálogo/UI se ve, dónde entra en el plan de fases, y cómo interactúa con el pipeline de pedidos/SLA/notificaciones existente.
...
14. **Cuenta de Instagram Business:** la integración automática (sección 13) requiere que el cliente tenga (o cree) una cuenta de Instagram Business/Creator vinculada a una página de Meta Business **antes** de la Fase 10 — sin eso no se puede generar el token de acceso inicial. **Estado:** confirmado que el cliente todavía no la tiene — el resto de la Fase 10 (Mi Cuenta, landing, banners, filtros de tienda, newsletter) se construyó igual; la sincronización de Instagram (`instagram_integration`, cron de renovación de token) queda pendiente hasta que exista la cuenta.
...
19. **Cuenta de WhatsApp Business:** igual que Instagram, la integración requiere que el cliente tenga (o cree) una cuenta de WhatsApp Business verificada vía Meta Business **antes** de la Fase 12 — sin eso no se puede enviar el mensaje inicial ni recibir comprobantes. **Estado:** confirmado que el cliente todavía no la tiene — el resto de la Fase 12 (cookies, baja/anonimización de cuenta, T&C con exención de retracto, Sentry, staging) se construyó igual; la transferencia bancaria por WhatsApp (mensaje inicial, webhook de comprobante, cola de revisión) queda pendiente hasta que exista la cuenta.
20. **Contenido legal (T&C, retracto, cookies):** la redacción exacta de la exención de retracto SERNAC y de la política de cookies es contenido legal que el cliente debe definir (idealmente con asesoría legal) antes del lanzamiento — el blueprint deja el mecanismo técnico listo (`terms_acceptances` versionado, `cookie_consents`) y en la Fase 12 se redactó un borrador funcional en `/terminos` (incluida la cláusula de exención de retracto para bienes perecibles/a pedido, Ley 19.496), pero **no reemplaza una revisión legal formal antes del lanzamiento**.
21. **Cuenta de Sentry:** se asume que el cliente crea una cuenta (el plan gratuito alcanza para v1) antes de la Fase 12; el DSN se configura como variable de entorno. **Estado:** confirmado y configurado en la Fase 12 (`instrumentation.ts`, `instrumentation-client.ts`, `global-error.tsx`), verificado con un evento de prueba real.
...
23. **Proyecto Supabase de staging:** creado en la Fase 12 (mismo esquema que producción, aplicado desde `supabase/apply-all-staging.sql`), con su propio `FIELD_ENCRYPTION_KEY`/`CRON_SECRET` y una sucursal de prueba — las credenciales de terceros en modo sandbox (Mercado Pago, Resend, Upstash, ORS) se reusan de `.env.local`. Vercel Preview Deployments deben configurarse para apuntar a este proyecto (pendiente, es una acción de configuración de Vercel, no de código).
