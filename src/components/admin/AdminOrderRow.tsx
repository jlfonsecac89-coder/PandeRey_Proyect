...
export function AdminOrderRow({
  ...
  return (
    <tr className="border-b border-charcoal-border align-top">
      <td className="py-2 pr-3 font-mono text-xs text-gold-dark">{order.id.slice(0, 8)}</td>
      <td className="py-2 pr-3 text-sm">{STATUS_LABELS[order.status as OrderStatus] ?? order.status}</td>
