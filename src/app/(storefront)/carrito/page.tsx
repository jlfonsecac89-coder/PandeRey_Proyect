...
export default function CarritoPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto max-w-2xl px-6 py-20 text-center">
        ...
        <Link
          href="/tienda"
          className="mt-6 inline-block rounded-full bg-gold px-6 py-2.5 text-sm font-semibold text-background shadow-card hover:bg-gold-hover"
        >
          Ir a la tienda
        </Link>
      </div>
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-10">
      ...
      <Link
        href="/checkout"
        className="mt-6 block w-full rounded-full bg-gold px-5 py-3 text-center text-sm font-semibold text-background shadow-card transition hover:bg-gold-hover"
      >
        Continuar al pago
      </Link>
    </div>
